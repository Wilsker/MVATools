...
void correlations( TString fin = "ttH_TMVA.root", Bool_t isRegression = kFALSE,
                   Bool_t greyScale = kFALSE, Bool_t useTMVAStyle = kTRUE )
{
   ...
   for (Int_t ic=0; ic<ncls; ic++) {
      ...
      TCanvas* c = new TCanvas( hName[ic], Form("Correlations between MVA input variables (%s)", (isRegression ? "" : (ic==0 ? "signal" : "background"))), ic*(width+5)+200, 0, width, width );
      Float_t newMargin1 = 0.15;
      Float_t newMargin2 = 0.19;
